...
<div class="template">
  <div class="box">
    ...
    <ul>
      @foreach($pics as $p)
      <li><a href="{{url('a/'.$p->art_id)}}"  target="_blank"><img src="{{url(config('app.thumb_path').'/'.$p->art_thumb)}}"></a><span>{{url($p->art_title)}}</span></li>
      @endforeach
    </ul>
  </div>
</div>
<article>
  ...
  @foreach($data as $d)
    <h3>{{$d->art_title}}</h3>
    <figure><img src="{{url(config('app.thumb_path').'/'.$d->art_thumb)}}"></figure>
    <ul>
      <p>{{$d->art_description}}</p>
      <a title="{{$d->art_title}}" href="{{url('a/'.$d->art_id)}}" target="_blank" class="readmore">阅读全文>></a>
    </ul>
    <p class="dateview"><span>{{date('Y-m-d',$d->art_time)}}</span><span>作者：{{$d->art_editor}}</span><span>个人博客：[<a href="{{url('a/'.$d->art_id)}}">{{$d->art_title}}</a>]</span></p>
@endforeach
  </div>
  <aside class="right">
    <!-- Baidu Button BEGIN -->
