...
@section('content')
<article class="blog">
<h1 class="t_nav"><span>{{$field->title}}</span><a href="/" class="n1">网站首页</a><a href="{{url('cate/'.$field->id)}}" class="n2">{{$field->name}}</a></h1>
<div class="newblog left">
    @foreach($data as $d)
   <h2>{{$d->title}}</h2>
   <p class="dateview"><span>发布时间：{{date('Y-m-d',$d->art_time)}}</span><span>作者：{{$d->art_editor}}</span><span>分类：[<a href="{{url('cate/'.$field->id)}}">{{$field->name}}</a>]</span></p>
